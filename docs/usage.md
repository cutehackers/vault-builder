...
`scripts/release_gate.sh`는 임시 복사본에서 tests, lint, health, maps, metrics, Stenc render/check를 실행합니다. 검증 중 생기는 report나 test artifact가 현재 vault를 오염시키지 않으며, 같은 gate는 `.github/workflows/release-gate.yml`에서도 실행됩니다.
